...
var Coin = class Coin {
    ...
    static create(pos) {
        basePos = pos.plus(new Vec(0.2, 0.1));
...
DOMDisplay.prototype.scrollPlayerIntoView = function(state) {
    ...
    // The viewport
    left = this.dom.scrollLeft, right = left + width;
    top = this.dom.scrollTop, bottom = top + height;
...
Level.prototype.touches = function(pos, size, type) {
    ...
    for (var y = yStart; y < yEnd; y++) {
        for (var x = xStart; x < xEnd; x++) {
            isOutside = x < 0 || x >= this.width ||
                y < 0 || y >= this.height;
            here = isOutside ? "wall" : this.rows[y][x];
...
Coin.prototype.update = function(time) {
    wobble = this.wobble + time * wobbleSpeed;
...
async function runGame(plans, Display) {
    for (level = 0; level < plans.length;) {
        status = await runLevel(new Level(plans[level]),
            Display);
        if (status == "won") level++;
    }
